...
公司推荐 provider 是 `xtalpi-pi-tools`，但它不是 `pi` 的硬依赖。没有晶泰 key、
DeepSeek key 或任何其他 key 时，`pi` 也必须先正常进入交互界面。用户随后在 Pi 内
使用 `/login` 登录任意受支持的 provider，再用 `/model` 选择模型；认证、模型选择和
下次启动恢复都由 upstream Pi 原生负责。
...
Windows 10/11 + Windows PowerShell 5.1 preflight
  ...
  -> 可选：隐藏输入公司 xtalpi key，允许直接回车跳过
  -> 完整 Windows acceptance
  -> 日常运行 pi
  -> 在 Pi 内按需使用 /login 和 /model
```
...
默认流程会询问一次可选的公司 xtalpi key。直接回车不会写入 provider/model 状态，
也不会影响安装和启动验收。如果希望完全跳过这个可选提示：
...
## 五、在 upstream Pi 内登录和选择模型

安装结束后，不管当前有没有任何 API key，都先运行：
...
1. `/login` 由 upstream Pi 保存对应 provider 的认证信息；
2. `/model` 由 upstream Pi 保存本次选择的 provider/model；
3. 正常退出后再次运行 `pi`，upstream Pi 自动恢复上次选择；
4. pi-67 不代理、不重写，也不会在 install/update 时根据 key 自动切换选择。

DeepSeek、Anthropic、OpenAI、Google 等 provider 都走上述 Pi 原生流程。不要为了
使用 upstream 内置 provider，手工在 `models.json` 中复制一份同名 provider。

公司用户如果希望在第一次启动 Pi 前提前写入晶泰 key，可以选择运行：

```powershell
pi-67 xtalpi configure --verify
```

这个命令只服务 `xtalpi-pi-tools` 公司 provider，使用隐藏输入；直接回车或
`--no-prompt` 且没有 key 时会成功跳过，不写 provider/model 状态。它不是运行
`pi` 的前置步骤，也不替代 `/login`、`/model` 或 upstream Pi 的持久化。

无人值守环境可通过以下任一 secret 环境变量提供晶泰 key，优先级从上到下：
...
不要把 key 写进 `.ps1`、README、Git 仓库或 CI 日志。
...
```powershell
.\scripts\pi67-windows-acceptance.ps1 -ValidateWorkstation -ProviderProfile auto
```
...
- `xtalpi-pi-tools` extension 在没有晶泰 key 时也能被 upstream Pi 正常加载；
- 当前 upstream Pi 选择的 provider/model 只读状态；
- `piStartupReady` 是启动硬门禁；`modelRequestReady` 是可选请求就绪状态；
- 只有当前 provider 已有可用认证时，才运行相应的真实请求或 xtalpi 专项 smoke。

也可以明确指定只读验收 profile；它只断言当前 upstream Pi 状态，不修改选择：

```powershell
.\scripts\pi67-windows-acceptance.ps1 -ProviderProfile unconfigured
.\scripts\pi67-windows-acceptance.ps1 -ProviderProfile xtalpi-pi-tools
.\scripts\pi67-windows-acceptance.ps1 -ProviderProfile deepseek
```

没有对应 key 或当前使用其他 provider 时，provider 专项阶段会显示 `SKIP`，例如：

```text
xtalpi-health
xtalpi-capability
xtalpi-smoke-self-test
xtalpi-live-smoke
xtalpi-summary-contract
```

这些是仅在条件满足时才适用的请求级检查；`SKIP` 是正确的不适用状态，不是启动
失败。最终仍以 `RESULT: PASS` 或 `RESULT: FAIL` 为准。
...
正常退出，重新运行 `pi`，确认 upstream Pi 自动恢复刚才的认证和模型选择。这个
人工步骤专门验收 upstream Pi 的原生持久化；pi-67 不参与写入。
...
不要把 npm、GitHub、晶泰 API、DeepSeek 官方 API 和公司代理问题混成一个“网络
问题”。它们是独立链路，应分别检查。
...
| 结果 | 含义 | 下一步 |
| --- | --- | --- |
| `PASS` | 工作站、Pi、pi-67 和 zero-key 启动/extension 合同通过；已配置 provider 时还会运行适用的请求级检查 | 重开 Terminal，运行 `pi`；需要时用 `/login`、`/model` |
