...
const BASH_WINDOWS_HOME_ACTION_EXAMPLE = JSON.stringify({
  kind: "tool_call",
  name: "bash",
  arguments: {
    command: 'ls -la "$HOME/.agents/skills"',
    timeout: 30,
  },
});
...
- Serialize JSON strings correctly: every double quote inside a string must be escaped. Never paste raw nested shell quotes into the "command" JSON string.
...
- In bash commands on Windows, prefer $HOME/... or forward-slash paths instead of raw C:\\Users\\... paths. Use this exact JSON encoding pattern when inspecting a home-relative directory: ${BASH_WINDOWS_HOME_ACTION_EXAMPLE}`;
