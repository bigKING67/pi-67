...
if [ -f "$NPM_PUBLISH_WORKFLOW" ]; then
  if grep -q "workflow_dispatch" "$NPM_PUBLISH_WORKFLOW" \
    ...
    && grep -q "publish-check --quiet --strict --no-pack" "$NPM_PUBLISH_WORKFLOW" \
