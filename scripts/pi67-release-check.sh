...
if [ -f "$XTALPI_PI_TOOLS_SCRIPT" ] && [ -f "$XTALPI_PI_TOOLS_TEST" ] && [ -f "$XTALPI_PI_TOOLS_SMOKE" ] && [ -f "$XTALPI_PI_TOOLS_DEBUG_SUMMARY" ] && [ -f "$XTALPI_PI_TOOLS_PROVIDER_HEALTH" ] && [ -f "$XTALPI_PI_TOOLS_ERROR_CONTRACT_CHECK" ] && [ -f "$XTALPI_PI_TOOLS_REPLAY_FIXTURES" ] && [ -f "$XTALPI_PI_TOOLS_ERROR_CONTRACT" ] && [ -f "$XTALPI_PI_TOOLS_DOC" ]; then
  pass "xtalpi-pi-tools launcher, tests, smoke, debug summary, provider health, error-contract check, fixtures, error contract, and docs exist"
else
  fail "xtalpi-pi-tools launcher, tests, smoke, debug summary, provider health, error-contract check, fixtures, error contract, or docs are missing"
fi
