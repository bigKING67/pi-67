...
if [ -f "$XTALPI_PI_TOOLS_SCRIPT" ] && [ -f "$XTALPI_PI_TOOLS_TEST" ] && [ -f "$XTALPI_PI_TOOLS_SMOKE" ] && [ -f "$XTALPI_PI_TOOLS_SMOKE_PS" ] && [ -f "$XTALPI_PI_TOOLS_DEBUG_SUMMARY" ] && [ -f "$XTALPI_PI_TOOLS_SMOKE_STATUS_CORE" ] && [ -f "$XTALPI_PI_TOOLS_SMOKE_PLAN" ] && [ -f "$XTALPI_PI_TOOLS_PROVIDER_HEALTH" ] && [ -f "$XTALPI_PI_TOOLS_ERROR_CONTRACT_CHECK" ] && [ -f "$XTALPI_PI_TOOLS_COVERAGE_AUDIT" ] && [ -f "$XTALPI_PI_TOOLS_REPLAY_FIXTURES" ] && [ -f "$XTALPI_PI_TOOLS_ERROR_CONTRACT" ] && [ -f "$XTALPI_PI_TOOLS_JSON_FILE" ] && [ -f "$XTALPI_PI_TOOLS_DOC" ]; then
  pass "xtalpi-pi-tools launcher, tests, Bash/PowerShell smoke, smoke plan, debug summary, smoke status core, provider health, error-contract check, coverage audit, fixtures, error contract, JSON helper, and docs exist"
else
  fail "xtalpi-pi-tools launcher, tests, Bash/PowerShell smoke, smoke plan, debug summary, smoke status core, provider health, error-contract check, coverage audit, fixtures, error contract, JSON helper, or docs are missing"
fi
...
if command_exists node; then
  ...
  warn "node not found; skipped JSON compatibility utility self-test"
fi
