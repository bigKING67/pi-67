...
RULES_LOADER_TEST="$REPO_ROOT/tests/pi-rules-loader/routing.test.mjs"
...
SMART_FETCH_CHARSET_PATCH="$REPO_ROOT/scripts/pi67-patch-pi-smart-fetch-charset.mjs"
...
if [ "${PI67_SKIP_RULES_LOADER_TEST:-0}" = "1" ]; then
  warn "Pi rules loader routing tests skipped for dependency-free artifact inspection"
elif command_exists node && [ -f "$RULES_LOADER_TEST" ]; then
  if RULES_LOADER_TEST_OUTPUT="$(node --no-warnings --test "$RULES_LOADER_TEST" 2>&1)"; then
    pass "Pi rules loader routing tests passed"
  else
    printf '%s\n' "$RULES_LOADER_TEST_OUTPUT" >&2
    fail "Pi rules loader routing tests failed"
  fi
else
  fail "Pi rules loader routing test requires node and tests/pi-rules-loader/routing.test.mjs"
fi
...
if command_exists node; then
  ...
  node --check "$SMART_FETCH_CHARSET_PATCH" >/dev/null
  node "$SMART_FETCH_CHARSET_PATCH" --self-test >/dev/null
  pass "JSON, MCP, and extension compatibility utilities passed syntax/self-tests"
else
  warn "node not found; skipped JSON and extension compatibility utility self-tests"
fi
