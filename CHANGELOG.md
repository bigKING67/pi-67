...
- `xtalpi-pi-tools` now uses local JSON action as the canonical default protocol through `local-action-adapter.ts`: it sends only plain Chat Completions plus `response_format={"type":"json_object"}` as a syntax hint, while keeping action schema validation, selected-tool allowlist, argument validation, shell guard, repair prompts, execution, and debug/smoke gates local to Pi.
- `xtalpi-pi-tools` JSON action mode now rejects legacy `<pi_tool_call>` markup as protocol drift and repairs it back to JSON action instead of silently falling back to the old text protocol. The old text protocol remains available only through explicit `XTALPI_PI_TOOLS_ACTION_PROTOCOL=legacy_text` regression/debug runs.
...
- `full-suite-strict` smoke trend gates now treat bounded local repair as an operationally acceptable adapter success path while continuing to gate empty finals, raw tool markup, process failures, provider errors, case-set drift, and excessive repair rates.
