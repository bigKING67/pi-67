...
当前发行版版本：`0.11.0`（见 `VERSION` 和 `CHANGELOG.md`）。
...
manager、团队工作台和个人 `xtalpi-pi-tools` key，并运行完整 Windows acceptance。
...
`pi-67 launch` 冒充真实 Pi 验收。只有工作站前置、真实 provider 和工具链都通过
才显示 `RESULT: PASS`，之后日常直接运行 `pi`。
