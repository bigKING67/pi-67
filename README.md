...
当前发行版版本：`0.11.3`（见 `VERSION` 和 `CHANGELOG.md`）。
...
```text
Windows 纯新机先准备 Terminal / PowerShell 7 / Notepad4 / Git / fnm / Node
  -> 安装 upstream Pi
  -> 安装 pi-67 manager
...
没有安装过 Windows Terminal、PowerShell 7、Notepad4、Git、Node.js、upstream
Pi 或 pi-67 的 Windows 电脑，不要从一长串手工命令开始。发布版提供单文件
`pi67-bootstrap.ps1`：
...
脚本会自动请求一次 UAC 管理员权限，并按固定顺序完成：WinGet 缺失修复、Windows
Terminal、系统内置 Windows PowerShell 管理员 profile、PowerShell 7、最终默认
PowerShell 7 管理员 profile、Notepad4 系统集成、Git 持久 PATH、fnm、
`lts/krypton`（Node.js 24 LTS），**最后才**安装真实 upstream Pi runtime、pi-67
manager 和团队工作台；公司 `xtalpi-pi-tools` key 只是可选配置，留空也会继续运行完整 Windows acceptance。
完整产品栈最低要求为 Node.js `>=22.19.0`，新机验收同时要求 major 24 且 active
`node.exe` 来自 fnm。

安全边界：脚本不永久修改 ExecutionPolicy 或系统代理；默认不修改 npm registry，
只有显式 `-UseNpmMirror` 才设置镜像；不接受明文 key 命令行参数；不会使用
`pi-67 launch` 冒充真实 Pi 验收。只要工作站、真实 `pi`、工作台和 extension 加载通过，
即使没有任何 provider key 也显示 `RESULT: PASS`；模型请求 readiness 作为独立状态报告。

完整的新机顺序、WinGet repair、Terminal `defaultProfile` / `elevate`、Notepad4
注册表、Git PATH、fnm profile、checksum、`-Minimal`、`-NoXtalpiPrompt`、日志和
分层网络排障见 [`docs/windows-fresh-install.md`](docs/windows-fresh-install.md)。
...
| **脚本** | `scripts/` | Windows bootstrap、configure、doctor、report、status、skill-audit、skill migration/sync/check、release artifact smoke、release、release-check、smoke、update、restore、uninstall、xtalpi-pi-tools 启动、测试和冒烟测试 |
...
直接运行发布版 bootstrap，不要求用户预先手工安装 npm 或 clone 仓库：
...
bootstrap 兼容 Windows PowerShell 5.1+，会请求一次 UAC 管理员权限。默认完整
安装 Windows Terminal、PowerShell 7、Notepad4、Git 和 fnm，通过
`fnm install/default/use lts/krypton` 准备 Node.js 24 LTS（最低 `22.19.0`），
最后才安装 Pi 和 pi-67。完整说明见
...
如果 `winget` 提示 Git 已安装但 `git --version` 仍找不到命令，直接继续执行：
...
`mcp.json` / `auth.json` / `image-gen.json` 缺失时从 `.example` 创建。若
in-place checkout 里只有 `settings.json` 等用户运行态配置是 tracked dirty，
updater 会比较 incoming changed paths；只有远端更新会碰到这些 dirty 运行态
